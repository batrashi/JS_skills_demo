...
for (let i = 0; i < lightCount; i++) {
    ...
    lightsArray.push(currentLight);
}

let cruiserSpeed = speed / 3.6;

for (let i = 0; i < lightsArray.length; i += 1) {
    let currentLight = lightsArray[i];
    let lightDistance = currentLight.distance;
    let lightDuration = currentLight.duration;

    let timeToCrossCurrentLight = lightDistance / cruiserSpeed;

    let switchCheck = timeToCrossCurrentLight % lightDuration;

    let colorCheck = Math.ceil(timeToCrossCurrentLight / lightDuration);
    let fromColorToColorCheck = (timeToCrossCurrentLight / lightDuration) % 2;

    if (switchCheck !== 0 && colorCheck % 2 === 0) {
        cruiserSpeed -= 1 / 3.6;
        i = -1;
    } else if (switchCheck === 0) {
        if (fromColorToColorCheck !== 0) {
            cruiserSpeed -= 1 / 3.6;
            i = -1;
        }
    }
...
print(`${Math.round(cruiserSpeed * 3.6)}`);